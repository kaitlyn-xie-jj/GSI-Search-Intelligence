...
SITL_PROFILE="${GSI_SITL_PROFILE:-GSI SearchWorld V1.1}"
...
status() {
    if docker inspect -f '{{.State.Running}}' "${CONTAINER_NAME}" 2>/dev/null \
        | grep -qx true; then
        docker inspect -f 'running container={{.Name}} started={{.State.StartedAt}} oom={{.State.OOMKilled}}' \
            "${CONTAINER_NAME}"
    else
...
case "${1:-foreground}" in
    ...
    status) status ;;
    stop) stop ;;
    *) echo "Usage: $0 {foreground|status|stop}" >&2; exit 2 ;;
esac
